...
client.devs = ['552103947662524416', "674813875291422720", "628595345798201355","616570697875193866"]
client.category = ['관리자', 'MODERATOR', 'INFO','노래', '돈 기능', '이벤트']
client.reloadCommands = async function () {
    client.commands.clear()
    client.aliases.clear()
 
    let cache = Object.keys(require.cache).filter(r => !r.includes('node_modules'))

    for (let key of cache) {
        delete require.cache[key]
    }
    for (let dir of (await fss.readdir(path.join(__dirname, 'commands')))) {
        (await fss.readdir(path.join(__dirname, 'commands', dir))).filter(r => r.endsWith('.js')).forEach(file => {
            const cmd = require(path.join(__dirname, 'commands', dir, file)) || {}
            if (!cmd.name) {
                console.warn(`Command file ${dir}/${file} doesn't have name field. skipping..`)
                return
            }
            if (!cmd.run) {
                console.warn(`Command file ${dir}/${file} doesn't have run field. skipping..`)
                return
            }
            cmd.category = dir
    ...
    return { commands: client.commands.size, aliases: client.aliases.size, cache: cache.length }
}
...
//https://discord.com/api/webhooks/776656057412878357/F8KsOfe9EGo5W99uf4FIqWwbeDUDoFgY743htgk1jMUNUTPGyE7KNDuxEUu1NC_KV-Ts
client.on("ready", () => {
...
client.on("message", async msg => {
    ...
    if (User[msg.author.id].xp > User[msg.author.id].level * 1000) {
        ...
        let levelup = new Discord.MessageEmbed()
            .setDescription(`***${msg.author.username}***님 ${User[msg.author.id].level}LVL로 레벨업 하였습니다!\n레벨업 보상: ${money}:coin:.`)
            .setColor("7289DA")
        msg.reply(levelup)
